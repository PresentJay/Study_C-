...
int getArrMinNext(int* src, int srcScale, int exception);
...
int main() {
	...
	do {
		...
		do {	// 무한루프 메뉴선택 (do로 먼저 선택, while (올바른 메뉴 선택할때까지)
			...
			printf("\n\t[NORMAL] : 알파벳을 누를 때마다, 지금까지 입력한 문자열과 함께 히스토그램(세로)을 출력하고,\n\t백스페이스 기능 추가!");
			printf("\n\t[HARD] : 알파벳을 누를 때마다, 지금까지 입력한 문자열과 함께 문자열 길이를 출력하고,\n\t히스토그램(세로)을 카운트 적은 순서로 출력하고, 백스페이스 기능 추가!\n\n");
			printf("\n\t어떤 버전의 히스토그램ver5을 시작할까요?\n\t(대소문자 상관없이 normal or hard)\n\n\t입력(exit 입력 시 종료)>> ");
			scanf("%s", &answer);
		...
		do {
			...
			if (version == 1) {
				...
				if (tmpMax > 0) {
					for (int i = tmpMax; i >= 0; i--) {
						for (int j = 'a'; j <= 'z'; j++) {
							...
							else if (alphaCount[j - 'a'] >= i)
								printf("*  ");
							else if (alphaCount[j - 'a'] < i && alphaCount[j - 'a'] > 0)
								printf("   ");
						}
						printf("\n\t");
					}
				...
				printf("입력한 문장 길이 = %d\n\n\t", strlen(str));
				tmpMax = getArrMaxNext(alphaCount, 26, tmpMax);
				tmpMin = getArrMinNext(alphaCount, 26, tmpMin);
				if (tmpMax > 0 && tmpMin > 0) {
					for (int k = tmpMax; k > 0; k--) {
						for (int i = 'a'; i <= 'z'; i++) {
							if (alphaCount[i - 'a'] > 0) {
								if (alphaCount[i - 'a'] < k) {
									printf("   ");
					...
					for (int k = tmpMin; k <= tmpMax; k = getArrMinNext(alphaCount, 26, k)) {
						for (int i = 'a'; i <= 'z'; i++) {
							if (alphaCount[i - 'a'] > 0)
								if (alphaCount[i - 'a'] == k)
									printf(" %c ", i);
			...
			else if (currentCh == VK_BACK) {
				if (alphaCount[str[strlen(str) - 1] - 'a'] > 0) {
					alphaCount[str[strlen(str) - 1] - 'a']--;
					str[strlen(str) - 1] = '\0';
				}
			}
...
int getArrMinNext(int* src, int srcScale, int exception) {
	int tmpMin = STRLEN;
	for (int i = 0; i < srcScale; i++)
		if (src[i] > exception && src[i] > 0) {
			tmpMin = MIN(tmpMin, src[i]);
		}
	return tmpMin;
}
